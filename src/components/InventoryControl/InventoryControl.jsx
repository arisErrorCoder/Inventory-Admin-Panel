...
const InventoryControl = () => {
    ...
    products.forEach((product) => {
      product.sizes.forEach((size) => {
        if (size.stock < size.thresholdStock) {
          const missingStock = size.thresholdStock - size.stock; // Calculate missing stock
          alerts.push({
            product: product.name,
            size: size.size,
            stock: size.stock,
            threshold: size.thresholdStock,
            missingStock: missingStock, // Add missing stock to the alert
          });
          toast.warn(
  ...
  const saveUpdatedThresholds = async () => {
    ...
    const isConfirmed = window.confirm("Are you sure you want to update the threshold limits?");
  
    if (!isConfirmed) {
      // If the user cancels, we exit the function
      return;
    }
  
    try {
      // Proceed with the update if confirmed
      const response = await axios.put(`${API_BASE_URL}/products/update-thresholds`, {
        products,
      });
      if (response.status === 200) {
        // Success message
        toast.success("Threshold limits updated successfully!", {
          position: "top-right",
        });
      }
    } catch (error) {
      ...
      toast.error("Failed to update threshold limits. Please try again.", {
        position: "top-right",
      });
    }
  };
